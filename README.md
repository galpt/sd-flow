...
The original scx_flow uses a **budget-driven tier system** with **rotating dispatch** to guarantee every task gets fair CPU time. sd-flow applies the same budget logic to diffusion: each step's sigma transition accumulates a noise budget, and that budget determines which solver the step gets — DDIM (high budget, deterministic, high quality), Euler (moderate budget), Euler Ancestral (low budget, adds variety), or Heun (deficit, extra compute for fine detail). The result is a sampler that allocates compute where the noise dynamics demand it most.
...
The solver per step is determined by the flow budget tier: high-budget steps (PRIORITY) get DDIM (deterministic, high quality at low step count), NORMAL steps get Euler (deterministic, 1st order ODE), LOW steps get Euler Ancestral (adds variety), and DEFICIT steps get Heun (extra compute for fine detail). This mirrors scx_flow's variable time-slice allocation. The solver also appears as `flow` in every built-in KSampler's sampler dropdown — no manual wiring needed.

### What's different from standard samplers?

| Aspect | Euler / Heun / DDIM | sd-flow `flow` sampler |
|--------|---------------------|------------------------|
| Solver behavior | Same for every step | Adaptive per budget tier (PRIORITY→DDIM, NORMAL→Euler, LOW→Euler_A, DEFICIT→Heun) |
