...
The schedule uses clean linear sigma spacing (same as ComfyUI's "Normal"
scheduler).  Per-step tier indices are computed from budget accumulation
and stored in ``step_tiers`` for the adaptive ``flow`` sampler.
"""
...
class FlowSigmaSchedule:
    ...
    def generate_schedule(self) -> torch.Tensor:
        """
        Generate the sigma schedule using linear spacing.
